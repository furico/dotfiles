...
Neovim 0.12 組み込みのパッケージマネージャ `vim.pack` を土台としたプラグイン管理基盤を定義する。外部マネージャ（lazy.nvim / packer / mini.deps 等）を導入せず、ランタイム標準だけでインストール・更新・ピン留め・複数マシン再現を成立させる。プラグインは `lua/config/plugins.lua` の `vim.pack.add` で宣言的に列挙し、薄い `init.lua` ローダから `require` 経由で読み込む。ロックファイルを Git 管理する。（配色は当初「検証用 colorscheme の起動時適用」要件で扱っていたが、`neovim-ui` の colorscheme 要件へ移管した。）
...
本 capability はプラグイン管理の土台（`vim.pack` レジストリ・ロックファイル・管理キーマップ）のみを扱い、以下を導入してはならない（MUST NOT）。これらは土台が立った後の別 change で扱う。

#### Scenario: 今回入れない項目

- **WHEN** 本 capability の成果物を確認する
- **THEN** LSP・補完・Treesitter・ファイラ・statusline 等の機能プラグインは含まれない
- **AND** `PackChanged` による build フックの本格運用は含まれない
