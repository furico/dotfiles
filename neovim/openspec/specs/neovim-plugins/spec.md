...
Neovim 0.12 組み込みのパッケージマネージャ `vim.pack` を土台としたプラグイン管理基盤を定義する。外部マネージャ（lazy.nvim / packer / mini.deps 等）を導入せず、ランタイム標準だけでインストール・更新・ピン留め・複数マシン再現を成立させる。プラグインは `lua/config/plugins.lua` の `vim.pack.add` で宣言的に列挙し、薄い `init.lua` ローダから `require` 経由で読み込む。ロックファイルを Git 管理し、検証用 colorscheme を 1 つ起動時適用する。
...
本 capability はプラグイン管理の土台と検証用 colorscheme のみを扱い、以下を導入してはならない（MUST NOT）。これらは土台が立った後の別 change で扱う。
