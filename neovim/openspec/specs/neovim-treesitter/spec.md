# neovim-treesitter Specification

## Purpose

`nvim-treesitter`（main ブランチ）を `vim.pack` で導入し、構文ハイライト・折りたたみ・（実験的）インデントを有効化する。main は組み込み `vim.treesitter` に土台を委譲する書き直し版で、プラグインの役割はパーサのインストーラと query 供給に縮小する（最低 nvim 0.12.0 必須）。treesitter 固有の設定は `neovim-plugins` の宣言レジストリ（`plugins.lua` の `vim.pack.add`）に登録しつつ、専用モジュール `lua/config/treesitter.lua` に分離して `require` 経由で読み込む。
