# neovim-git Specification

## Purpose

`lewis6991/gitsigns.nvim` による git 統合を定義する。git の変更（追加・変更・削除）を sign 列に表示し、`on_attach` で git 管理下のバッファにのみ hunk ナビ（`]c`/`[c`、diff モード時は組み込みへフォールバック）と `<leader>h` 名前空間の hunk 操作（stage/reset/preview）・blame をバッファローカルに付ける。設定は専用モジュール `lua/config/git.lua` に分離し、`plugins.lua` の `vim.pack.add` レジストリに登録する。純 Lua で build 不要、未導入でも起動を壊さない。
