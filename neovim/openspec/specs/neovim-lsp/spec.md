...
nvim 0.12 組み込みの `vim.lsp`（`vim.lsp.config` / `vim.lsp.enable`、デフォルト LSP キーマップ `grn`/`gra`/`grr`/`K` 等）を土台に LSP を有効化する。サーバ調達は `mason.nvim`、橋渡しと自動有効化は `mason-lspconfig.nvim`（v2）、各サーバの設定データは `nvim-lspconfig` が供給する。設定は専用モジュール `lua/config/lsp.lua` に分離し、`plugins.lua` の `vim.pack.add` レジストリに登録する。補完エンジン（`neovim-completion`）の capability を全サーバへ配線する。
