...
└── .config/nvim/
    ...
    └── lua/config/
        ...
        ├── lazygit.lua          snacks.lazygit（lazygit 連携）のキーマップ
...
- **which-key**: キー押下途中に割当てをポップアップ表示。`<leader>p`=plugins /
  `<leader>h`=hunks / `<leader>f`=find / `<leader>g`=git のグループ名を登録（find の実キーマップは
...
**picker モジュール**で行う。純 Lua・**build 不要**・外部バイナリ不要で、
`vim.pack.add` は `version` を省略して default ブランチを使う。
require は pcall 保護で、未導入でも起動を壊さない。

- **setup の集約**: snacks の `setup` は中央モジュール `snacks.lua` が1か所で行う
  （`require("snacks").setup({ picker = { enabled = true }, explorer = { enabled = true } })`）。
  snacks の各モジュールは単一 setup を共有するため、`finder.lua` は picker のキーマップ、
  `explorer.lua` は explorer のキーマップだけを持つ薄い層にする。dashboard / notifier /
  scroll / indent 等の他モジュールは列挙せず既定の無効のまま保つ。`plugins.lua` では
  `snacks`（setup）→ `finder` → `explorer` の順に require する。
...
ディレクトリ構造の俯瞰とファイル操作は `folke/snacks.nvim` の **explorer モジュール**で
行う（picker の上に構築されるサイドバー型ツリー）。有効化は中央 setup（`snacks.lua`）、
キーマップは `explorer.lua` に置く。新規プラグインは増やさない（snacks は finder で導入済み）。

- **既定をそのまま採用**: `replace_netrw = true`（`nvim <ディレクトリ>` で netrw の代わりに
  explorer が開く）、`trash = true`（削除はシステムのゴミ箱へ。`trash`/`gio` 等のコマンドが
  あれば実効。無ければ完全削除へフォールバック）。tree 表示・git 差分・LSP 診断のツリー
  表示・現在ファイルの自動追従・sidebar レイアウトも既定のまま。
- **キーマップ**: `desc` 付き・押下時に snacks を遅延参照。

  | キー | 機能 |
  |------|------|
  | `<leader>e` | ファイラを開閉 |
  | `<leader>E` | 現在ファイルをファイラで表示（reveal） |

  単独キーのため which-key グループ登録は不要（`desc` がそのまま表示される）。ツリー内の
  ファイル操作キーは snacks の既定に委ねる（抜粋）: `a` 作成 / `d` 削除 / `r` 改名 /
  `c` コピー / `m` 移動 / `y` ヤンク / `p` ペースト、`h` 折りたたみ / `l` 展開 /
  `<BS>` 親へ、`H` 隠しファイル切替 / `I` ignore 切替、`]g`/`[g` git hunk 間 /
  `]d`/`[d` 診断間、`<leader>/` その場で grep。

### lazygit 連携（snacks.lazygit）

git 操作は `folke/snacks.nvim` の **lazygit モジュール**で行う。neovim 内のフロートで
lazygit を開き、有効化は不要（`Snacks.lazygit()` を呼ぶだけ）、キーマップは `lazygit.lua`
に置く。新規プラグインは増やさない（snacks は finder で導入済み）。

- **neovim へ開き直す連携**: lazygit でファイルにカーソルを合わせ `e`（edit）を押すと、
  今使っている**同じ neovim インスタンス**にそのファイルが開く（`$NVIM` サーバ経由）。
  lazygit で見た変更をそのまま全体閲覧・編集できる。diff の配色は git パッケージの
  `~/.gitconfig`（delta / Catppuccin Mocha）をそのまま使う（snacks が触るのは lazygit の
  UI テーマのみで、diff pager=delta は据え置き）。
- **キーマップ（`<leader>g`=git）**: `desc` 付き・押下時に snacks を遅延参照。

  | キー | 機能 |
  |------|------|
  | `<leader>gg` | lazygit を開く |
  | `<leader>gf` | 現在ファイルの履歴（log_file） |
  | `<leader>gl` | リポジトリの履歴（log） |

  `<leader>g`=git のグループ名は `ui.lua` の which-key に登録（group 名は ui.lua、
  実キーマップは lazygit.lua という finder / hunks と同じ分離）。tmux popup 起動
  （`prefix + C-g`）の standalone lazygit も従来どおり併用できる。
