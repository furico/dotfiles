...
tmux のプラグイン管理方針を定義する。TPM を足場として導入しつつ、本構成では追加プラグインを入れず、その理由を明示する。
...
本構成では追加プラグインを導入しない SHALL（TPM は将来のための足場として残す）。以下はいずれも入れない: シームレス移動（`vim-tmux-navigator`）、永続化（`tmux-resurrect`/`tmux-continuum`）、配色テーマ、`tmux-yank`。理由を README もしくは設定コメントに残す。
